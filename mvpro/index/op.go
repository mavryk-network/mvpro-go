...
type Op struct {
	...
	PrevBaker     Address             `json:"previous_baker"            mvpro:"-"` // delegation
	Source        Address             `json:"source"                    mvpro:"-"` // internal operations
	Offender      Address             `json:"offender"                  mvpro:"-"` // double_x
	Accuser       Address             `json:"accuser"                   mvpro:"-"` // double_x
	Loser         Address             `json:"loser"                     mvpro:"-"` // smart rollup refutation game
	Winner        Address             `json:"winner"                    mvpro:"-"` // smart rollup refutation game
	Staker        Address             `json:"staker"                    mvpro:"-"` // smart rollup refutation game
	Solution      mavryk.HexBytes     `json:"solution,omitempty"        mvpro:"-"` // smart rollup refutation game
	Proof         mavryk.HexBytes     `json:"proof,omitempty"           mvpro:"-"` // smart rollup refutation game
	Storage       json.RawMessage     `json:"storage,omitempty"         mvpro:"-"` // transaction, origination
	Script        *Script             `json:"script,omitempty"          mvpro:"-"` // origination
	Power         int                 `json:"power,omitempty"           mvpro:"-"` // endorsement
	Limit         *float64            `json:"limit,omitempty"           mvpro:"-"` // set deposits limit
	Confirmations int64               `json:"confirmations"             mvpro:"-"`
	NOps          int                 `json:"n_ops,omitempty"           mvpro:"-"`
	Batch         []*Op               `json:"batch,omitempty"           mvpro:"-"`
	Internal      []*Op               `json:"internal,omitempty"        mvpro:"-"`
	Metadata      map[string]Metadata `json:"metadata,omitempty"        mvpro:"-"`
	Events        []Event             `json:"events,omitempty"          mvpro:"-"`
	TicketUpdates []TicketUpdate      `json:"ticket_updates,omitempty"  mvpro:"-"`
