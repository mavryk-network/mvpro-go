...
func getTypeInfo(v any) (*TypeInfo, error) {
	...
	if val.Kind() == reflect.Pointer {
		return getReflectTypeInfo(val.Type().Elem(), tagName)
	}
	if !val.IsValid() {
...
func structFieldInfo(f *reflect.StructField, tagname string) *FieldInfo {
	...
	switch tags := strings.Split(f.Tag.Get(flagName), ","); tags[0] {
	case "-":
		// ignore
		return nil
	case "":
		// use json tag name
		switch tags := strings.Split(f.Tag.Get(tagname), ","); tags[0] {
		case "":
